...
class MainWindow:
    ...
    def update_dependent_widgets_state(self, *args):
        ...
        use_cursor_enabled = self.dig_tool.param_vars.get('use_custom_cursor', tk.BooleanVar()).get()

        if auto_walk_enabled:
            if self.auto_shovel_checkbox: self.auto_shovel_checkbox.config(state='normal', fg="#000000")
            if self.auto_sell_checkbox: self.auto_sell_checkbox.config(state='normal', fg="#000000")
        else:
            if self.auto_shovel_checkbox: self.auto_shovel_checkbox.config(state='disabled', fg="#666666")
            self.dig_tool.param_vars['auto_shovel_enabled'].set(False)
            
            if self.auto_sell_checkbox: self.auto_sell_checkbox.config(state='disabled', fg="#666666")
            self.dig_tool.param_vars['auto_sell_enabled'].set(False)

        shovel_enabled = self.dig_tool.param_vars.get('auto_shovel_enabled', tk.BooleanVar()).get()
        ...
        if self.use_cursor_checkbox:
            if auto_walk_enabled:
                self.dig_tool.param_vars['use_custom_cursor'].set(False)
                self.use_cursor_checkbox.config(state='disabled', fg="#666666")
            else:
                self.use_cursor_checkbox.config(state='normal', fg="#000000")
