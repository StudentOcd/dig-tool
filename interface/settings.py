...
class DigTool:
    ...
    def _toggle_auto_sell_thread_safe(self):
        if not self.param_vars['auto_walk_enabled'].get():
            self.update_status("Enable Auto-Walk before enabling Auto-Sell.")
            return

        if not self.automation_manager.sell_button_position:
            self.update_status("Set Sell Button position before enabling Auto-Sell.")
            return
